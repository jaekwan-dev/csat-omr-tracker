...
import { SUBJECT_CONFIG, type Subject, type QuestionInput } from "@/lib/exam-data"
...
export default function EditExamPage() {
    ...
    fetch(`/api/teacher/exams/${examId}`)
      ...
      .then((d) => {
        if (d.exam) {
          ...
          setExistingPdfUrl(d.exam.explanationPdfUrl ?? null)
        }
      })
      .catch(() => {})
  ...
  async function handleSubmit() {
    ...
    setError("")
    setSubmitting(true)
    let explanationPdfUrl: string | null = existingPdfUrl
    ...
    try {
      ...
      toast.success(`"${title.trim()}" 시험이 수정되었습니다.`)
  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center gap-3">
        ...
        <span className="text-sm text-muted-foreground">시험 정보를 불러오는 중...</span>
      </div>
    )
  }
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold tracking-tight">시험 수정</h1>
              <p className="text-xs text-muted-foreground">
                {cfg.label} · 총 {questions.length}문항
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6 md:col-span-5">
            <Card>
              ...
              <CardContent>
                <FieldGroup>
                  <Field>
                    <FieldLabel>과목 (수정 불가)</FieldLabel>
                    <SubjectPicker value={subject} onChange={() => {}} disabled />
                  </Field>
                  ...
                  <Field>
                    <FieldLabel htmlFor="explanationFile">
                      해설지 PDF {existingPdfUrl ? "(새 파일 업로드 시 교체됨)" : "(선택)"}
                    </FieldLabel>
                    <label
                    ...
                    >
                      ...
                      <p className="text-xs text-muted-foreground">
                        {explanationFile ? (
                          ...
                          <span className="font-medium text-primary">등록된 해설지가 있습니다 · 클릭하여 변경</span>
                        ) : (
                          "클릭하여 PDF 파일 업로드"
                        )}
                      </p>
